...
export default function VoiceMessage({ url, duration = 0, isSelf = false }: VoiceMessageProps) {
  ...
  const totalDuration =
    player.duration && player.duration > 0 ? Math.round(player.duration) : duration;
  ...
  }, [player.currentTime, player.duration, progressAnim]);
  ...
  return (
    ...
    >
      ...
      <View className="flex-1 gap-1">
        <View
          className={`h-1.5 rounded-full overflow-hidden ${isSelf ? 'bg-white/30' : 'bg-gray-300'}`}
        >
          <Animated.View
